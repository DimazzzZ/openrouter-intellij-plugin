...
- **Proxy Passthrough Preserves OpenRouter-Specific Fields (#56)** - The chat completion proxy deserialized incoming requests into the typed `OpenAIChatCompletionRequest` and re-serialized them, silently dropping any field the typed model does not declare — losing OpenRouter-specific parameters (`provider`, `models[]` fallback, `route`, `transforms`, `response_format`, `plugins`, `preset`, `usage`). The outbound body is now built from the raw `JsonObject` alongside the typed model (still used for validation/logging/multimodal checks), so all fields — including any future OpenRouter parameter — survive without code changes. `max_tokens` is injected only when absent, and `x-openrouter-*` response headers are logged at debug level on both streaming and non-streaming paths
- **Tool-Call-Only Responses Accepted (#72)** - `ResponseTranslator` validation rejected assistant messages whose only payload was `tool_calls` (empty `content`), which is how tool-calling models return the first turn of an agent step. Validation now accepts a message that carries either `content` **or** `tool_calls`, unblocking AI Assistant tool-use flows through the proxy
- **Proxy Port Fallback Aligned to 8880** - The private fallback constants in `OpenRouterConfigurable` (settings field placeholder) and `AIAssistantIntegrationHelper` (sample URL shown when the server is not running) were still `8080`, but the runtime default is `8880` (`SetupWizardConfig.DEFAULT_PROXY_PORT` / `OpenRouterSettings.proxyPortRangeStart`). User-facing UI now advertises the port the server actually binds to
- **Plugin Availability Check & Credential Attributes** - `AIAssistantIntegrationHelper` plugin-availability probe was simplified (dropped brittle reflection paths that could false-negative on newer IDEs), and `PasswordSafeKeyStorage` now constructs `CredentialAttributes` via the modern 3-arg constructor everywhere (no more deprecated default-argument synthetic)

### Build & Tooling
- **Platform Floor Raised to 2025.3 (#63, ADR-0005)** - `pluginSinceBuild` moved from `242` to `253`, the IntelliJ Platform target moved from `2024.2` to `2025.3.6`, and the README/DEVELOPMENT prereqs were refreshed to match. Rationale: JetBrains unified the IDEA distribution starting with 2025.3 (no separate IC artifact for build 253+), IPGP 2.18.x toolchain drift, and IPGP-recommended API deprecations reworked between 2024.2 and 2025.3
- **Gradle 9.4.0 · Kotlin 2.1.20 · IntelliJ Platform Gradle Plugin 2.18.1 (#57)** - Upgraded the build toolchain. IPGP 2.18.1 removed the `ides(List<String>)/ide(String)` overloads, so the `pluginVerification` block was restructured to `create(Provider<List<String>>)` and gained CLI-overridable IDE selection: `-PverifierLocalIde=<path>` (local loop), `-PverifierIdes=IC-2025.3` (pinned PR CI), or `recommended()` (release CI sweep) — an empty `-PverifierIdes` fails loudly instead of silently widening. `failureLevel` now gates the whole non-public-API surface (internal / experimental / scheduled-for-removal, etc.), with `DEPRECATED_API_USAGES` excluded per verifier report analysis. A daemon JVM pin (`gradle-daemon-jvm.properties = 21`) fixes the detekt abort when the launcher JDK is newer than 21. Also fixed an OkHttp `TaskRunner` thread leak in `OpenRouterProxyServerTest` that the stricter `ThreadLeakTracker` in IPGP 2.18.1 caught
- **Gradle Wrapper Refresh (#67)** - Bumped the Gradle wrapper (`gradlew`, `gradlew.bat`, `gradle-wrapper.jar`, `gradle-wrapper.properties`) to the current pinned version
- **Detekt Gates the Build (#58)** - Removed `ignoreFailures = true` from the built-in `detekt` task; a finding now fails the build. Dropped the redundant custom `detektSarif` task (detekt was running twice with a stale exclude list) and configured the built-in task to emit SARIF + txt + html so GitHub Code Scanning still gets its upload. Deleted the empty `baseline.xml` — a real baseline is only reintroduced if a batch of pre-existing findings needs quarantining
- **CI Workflows Ported from token-pulse (#59)** - Restructured `ci.yml` into three jobs (`test` running `test + koverVerify + detekt`, `verify` running `verifyPlugin` against the newest supported IDE resolved dynamically from `printProductsReleases`, and `results` posting a summary comment when detekt found issues). Java 17 → 21 to match `build.gradle.kts`; concurrency group cancels superseded PR runs; `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24` opts into Node.js 24; cache strategy is main-writes / PR-reads to avoid cache bloat; Plugin Verifier cache pinned at `~/.pluginVerifier`. `release.yml` runs the unpinned `recommended()` verifier sweep, validates the built plugin archive, and gates on required Marketplace secrets with helpful error messages
- **Testing Infrastructure Parity with token-pulse (#61)** - Ported the ADR-driven testing conventions: `*PlatformTest` / `*SmokeTest` class-name suffixes select platform-bound tests, off-platform unit tests run under a slim detekt config, and `OkHttpLeakSafeExtension` shuts down OkHttp executors after platform tests to satisfy IPGP 2.18.1's `ThreadLeakTracker`. Added ADR-0001 (min platform 2024.2), ADR-0002 (no non-public platform API), ADR-0003 (disabled tests → tagged taxonomy), plus `docs/agents/` (domain, issue-tracker, triage-labels) and `scripts/fast-build.sh` for the local build/verify loop
...
- [`docs/adr/0005-raise-minimum-platform-to-2025-3.md`](docs/adr/0005-raise-minimum-platform-to-2025-3.md) — why the plugin's minimum platform was raised from 2024.2 to 2025.3 (supersedes ADR-0001)
- [`NOTICE`](NOTICE) (#69) — trademark attribution crediting the OpenRouter name and logos to OpenRouter, Inc, with a manifest of the bundled branded assets (per <https://openrouter.ai/brand>); README footer updated to match
